...
function stationsToHTML($tableDeStations){
    $res = '<table><thead><tr><td>nom</td><td>marque</td><td>adresse</td><td>nombre d\'avis recus</td><td>Note globale</td><td>note Accueil</td><td>note services</td><<td>note prix</td>/tr></thead><tbody>';
    foreach ($tableDeStations as $station) {
        $res .= "<tr><td>{$station->nom}</td><td>{$station->marque}</td>
        <td>{$station->adresse}</td><td>{$station->nbnotes}</td>
        <td>{$station->noteglobale}</td><td>{$station->noteaccueil}</td>
        <td>{$station->services}</td><td>{$station->prix}</td>
        </tr>";
    }
    $res .= '</tbody></table>';
    return $res;
